...
<header class="container">
    <div class="header__wrapper">
        <div class="header__social">
            <a target="_blank" href="<?=$COMPANY->socials['vk']['href']?>" class="text-uppercase d-flex align-items-center" title="<?=$COMPANY->socials['vk']['name']?>">
                <img class="social-icon" src="/assets/icons/social-vk.svg" alt="ВКонтакте">
                <span><?=$COMPANY->socials['vk']['title']?></span>
            </a>
            <a target="_blank" href="<?=$COMPANY->socials['instagram']['href']?>" class="text-uppercase d-flex align-items-center" title="<?=$COMPANY->socials['instagram']['name']?>">
...
<nav class="header-nav">
    <div class="container header-nav-main">
        <div class="header-nav__pages">
            <?php foreach($PAGE->all as $key => $page):?>
                <?php if($page['isMenuPage']):
                    ...
                    ?>
                    <div class="item<?=$page['fileName'] === $PAGE->current ? ' open' : null?>">
        ...
        <div class="header-nav__icons">
            <div class="item position-relative d-flex justify-content-center">
                <a class="item__data" href="/lk">
                    ...
                    <span class="item__data__title"><?php if(!$USER->isGuest()){echo($USER->get(["name"]));}else{echo("Вход");}?></span>
                </a>
                <?php if(!$USER->isGuest()):?>
            ...
            foreach($headerAdd as $href => $item):?>
                <div class="item mobile">
                    <a class="item__data" href="<?=$href?>">
                        ...
                        <?php if($href === '/cart'):?><span class="cart_number"><?=$CART->getCount($USER->getId())?></span><?php endif;?>
                    </a>
                    <?php if($href === '/lk' &&!$USER->isGuest()):?>
                        <div class="lk-menu shadowBox">
                            <a href="/lk" class="item" title="Личный кабинет">Личный кабинет</a>
                            <a href="/lk/orders" class="item" title="Заказы">Заказы</a>
                            <span class="item lk-menu-exit" title="Выйти">Выйти</span>
