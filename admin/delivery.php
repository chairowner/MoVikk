...
$execute = [];
if (isset($editId)) {
    $data = $_DELIVERIES->Get($editId);
    if (!isset($data['items'][0])) $_PAGE->Redirect("admin/$editCategory");
} else {
    if (isset($_GET['search']) && trim($_GET['search']) !== "") {
        $search = trim($_GET['search']);
        $data = "SELECT * FROM `{$_DELIVERIES->GetTable()}` WHERE `name` LIKE :search OR `link` LIKE :search LIMIT ".(($currentPageNumber - 1) * $viewCount).", ".$viewCount;
        $execute['search'] = "%$search%";
    } else {
        $data = "SELECT * FROM `{$_DELIVERIES->GetTable()}` LIMIT ".(($currentPageNumber - 1) * $viewCount).", ".$viewCount;
    }
...
if (isset($editId)) {
    ...
    if (isset($search)) {
        ...
        $all_count = "SELECT COUNT(`id`) `counter` FROM `{$_DELIVERIES->GetTable()}` WHERE `name` LIKE :search OR `link` LIKE :search";
        $all_count_execute = ['search' => "%$search%"];
    } else {
        $pattern .= "/admin/$editCategory?page=#";
        $all_count = "SELECT COUNT(`id`) `counter` FROM `{$_DELIVERIES->GetTable()}`";
    }
                ...
                <div id="main">
                    ...
                    <?php if ($action === "add"): // создание?>
                        ...
                        <form method="POST" action="/admin/actions/<?=$editCategory?>/add" id="formData">
                            ...
                            <label class="item">
                                <span>Ссылка на доставку<span class="error">*</span></span>
                                <input type="text" name="link" require class="field" maxlength="255">
                            </label>
                    ...
                    <?php else:?>
                        ...
                        <?php if (isset($editId)): // редактирование?>
                            ...
                            <form method="POST" action="/admin/actions/<?=$editCategory?>/edit" id="formData">
                                ...
                                <label class="item">
                                    <span>Ссылка на доставку<span class="error">*</span></span>
                                    <input type="text" name="link" require class="field" maxlength="255" value="<?=$item['link']?>">
                                </label>
                        ...
                        <?php else: // вывод всех элементов?>
                            ...
                            <form id="form-search">
                                <div class="search">
                                    <input type="text" name="search" placeholder="Поиск доставки" value="<?=$search?>">
                                </div>
                            </form>

                            <a class="button shadowBox" href="/admin/<?=$editCategory?>?action=add">Добавить доставку</a>
                                        ...
                                        <a class="item shadowBox" href="/admin/<?=$editCategory?>?action=edit&id=<?=$item['id']?>">
                                            <div class="item-content d-flex flex-column">
                                                ...
                                                <?php if(isset($item['link'])):?>
                                                    <span class="d-flex flex-row flex-wrap gap-5">(<?=$item['link']?>)</span>
                                                <?php endif;?>
                                            </div>
                                        </a>
                            ...
                            <?php else:?>
                                ...
                                <p class="text-center">Не найдено ни одной доставки</p>
